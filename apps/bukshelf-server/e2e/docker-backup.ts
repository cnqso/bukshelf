...
const env = {
  ...
  // Bun container. The test never calls either provider.
  OPENROUTER_API_KEY: 'docker-e2e-openrouter-key',
  SONIOX_API_KEY: 'docker-e2e-soniox-key',
};
...
try {
  ...
  const capabilities = (
    (await discovery.json()) as {
      capabilities: { readerAI: boolean; textToSpeech: boolean; usageMetering: boolean };
    }
  ).capabilities;
